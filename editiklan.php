...
if (isset($_POST['ubah'])) 
{
	...
	if (!empty($lokasi)) 
    {
    	// jika gambar di ganti maka simpan gambar baru
        move_uploaded_file($lokasi, "gambar/".$gambar);
        $update=$koneksi->query("UPDATE  iklan set nm_perusahaan='$nm_perusahaan', isi_iklan='$isi_iklan',gambar='$gambar',tgl_mulai='$tgl_mulai',tgl_akhir='$tgl_akhir' ,hargasewa='$hargasewa', lamasewa='$lamasewa', totalharga='$totalharga', aktif=1, email='$email', link='$link' WHERE id_iklan='$id_iklan'") or die(mysqli_error($koneksi));
        ...
        else
        {
            ...
            echo "<meta http-equiv='refresh' content='1;url=editiklan.php'>";  
        }
    }
    else
    { 
    	// jika gambar tidak di ganti
        $update=$koneksi->query("UPDATE  iklan set nm_perusahaan='$nm_perusahaan', isi_iklan='$isi_iklan',tgl_mulai='$tgl_mulai',tgl_akhir='$tgl_akhir', hargasewa='$hargasewa', lamasewa='$lamasewa', totalharga='$totalharga', aktif=1, email='$email', link='$link' WHERE id_iklan='$id_iklan'") or die(mysqli_error($koneksi));
